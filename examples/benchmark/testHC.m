%% Benchmark Evaluation for HC Method
% This script evaluates the Histogram-based Contrast (HC) method on a benchmark dataset.
...
fprintf('=== HC Method Benchmark Evaluation ===\n');
fprintf('Dataset: %s\n\n', dirName{1});
...
if ~isempty(precisionList)
    ...
    csvwrite(fullfile(outputDir, 'precision_HC.csv'), precisionList);
    csvwrite(fullfile(outputDir, 'recall_HC.csv'), recallList);
    csvwrite(fullfile(outputDir, 'precision_average_HC.csv'), averagePrecision);
    csvwrite(fullfile(outputDir, 'recall_average_HC.csv'), averageRecall);
    
    %% Plot Results
    figure('Name', 'HC Method Benchmark Results', 'Position', [100, 100, 1000, 400]);
    ...
    xlabel('Image Index');
    ylabel('Score');
    title('HC Method: Precision and Recall per Image');
    ...
    ylabel('Score');
    title('HC Method: Average Metrics');
    ylim([0, 1]);
    grid on;
    ...
    % Save figure
    saveas(gcf, fullfile(outputDir, 'benchmark_HC.png'));
